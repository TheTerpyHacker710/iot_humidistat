...
def main():
    parser = argparse.ArgumentParser(
        description="Gordon Deacon CMP408 Humidistat IoT w/ AWS Project"
    )
    parser.add_argument(
        "--broker",
    ...
    print(
        f"""Reads Enviro data and sends over mqtt.
    ...
    client = paho.Client(client_id=device_id, transport='websockets')
    ...
    while True:
        ...
        img = overlay_text(background, (0 + margin, 0 +
                           margin), time_string, font_lg)
        ...
        try:
            values = {
                "temperature": round(corr_temperature, 2),
                "pressure": round(mean_pressure/1000, 2),
                "humidity": round(corr_humidity, 2),
                "light": round(light, 2)
            }
            values["serial"] = device_serial_number
            print(values)
            client.publish(args.topic, json.dumps(values))
